...
class RepositoryInspector:
    ...
    def inspect(self, repo: Mapping[str, Any], tree: Iterable[Mapping[str, Any]]) -> RepositoryInspection:
        ...
        revision = str(repo.get("default_branch") or "")
        return RepositoryInspection(
            repository=str(repo.get("full_name") or repo.get("name") or ""),
            revision=revision,
            tree_hash=str(repo.get("tree_sha") or "NOT_MEASURED"),
...
class AssetDiscoveryEngine:
    ...
    def inspect_candidate(self, candidate: Mapping[str, Any]) -> RepositoryInspection:
        ...
        revision = str(repo.get("default_branch") or "")
        tree = self.provider.tree(full_name, revision)
        # A provider may supply an immutable tree SHA in the repository payload.
        if repo.get("tree_sha"):
            repo = dict(repo)
        return self.inspector.inspect(repo, tree)
